...
import './VDD.css'; // Tệp CSS cho kiểu dáng
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faArrowRight, faCalendar, faCheck, faCheckCircle, faComment, faExclamationCircle, faPhone, faTimes } from '@fortawesome/free-solid-svg-icons';
...
function VDD() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const quanlyRef = await getDocs(collection(firestore, "VDD"));
    ...
    <div className="container-fluid">
      ...
      <div className="col-md-3 left-side" style={{ width: '250px' }}>   
        ...
        <div className="col-md-10 right-side" style={{
          ...
          <h3 style={{color:"#EC008C"}}>{data.map((item, index) => (
                  <div key={index} >
          ...
          <p>{data.map((item, index) => (
                  <div key={index} >
                                    {data.map((item, index) => (
                              <div key={index}>
                                 {item.ND3} <Link to="/" className="link">tại đây</Link>.
                              </div>
                            ))} 
                  </div>
                ))} </p>
